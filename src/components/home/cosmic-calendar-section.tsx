...
export function CosmicCalendarSection() {
  ...
  return (
    ...
    >
      <Card className="relative overflow-hidden border-padma-champagne/35 bg-gradient-to-br from-white/95 via-padma-cream/90 to-padma-lavender/15 shadow-soft">
        {!hyperfocus && (
          <div className="pointer-events-none absolute -left-20 top-0 h-56 w-56 rounded-full bg-padma-champagne/20 blur-3xl" aria-hidden />
        )}
        <CardHeader className="relative">
          <div className="flex flex-wrap items-center gap-2">
            <CalendarRange className="h-5 w-5 text-padma-lavender" aria-hidden />
        ...
        <CardContent className="relative space-y-6">
          {/* Bande des 30 jours */}
          <div>
            <p className="mb-3 flex items-center gap-2 font-display text-xs uppercase tracking-[0.2em] text-padma-pearl">
          ...
          {/* Transits */}
          <div className="rounded-2xl border border-padma-pearl/40 bg-white/55 p-4">
            <p className="mb-3 flex items-center gap-2 font-display text-xs uppercase tracking-[0.2em] text-padma-pearl">
            ...
            <ul className="space-y-3">
              {transits.length === 0 && (
                <li className="text-sm text-padma-night/65">
              ...
              {transits.map((t) => (
                <li
                  ...
                  className="rounded-xl border border-padma-champagne/25 bg-padma-cream/40 px-3 py-2"
                >
                  <p className="text-xs font-medium uppercase tracking-wide text-padma-pearl">
                    {t.dateLabel}
                  </p>
                  <p className="font-display text-sm text-padma-night">{t.title}</p>
                  <p className="mt-1 text-xs leading-relaxed text-padma-night/70">{t.detail}</p>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div className="rounded-2xl border border-padma-lavender/35 bg-gradient-to-br from-padma-lavender/12 to-transparent p-4">
            <p className="mb-2 flex items-center gap-2 font-cinzel text-sm tracking-wide text-padma-night">
              <Sparkles className="h-4 w-4 text-padma-champagne" aria-hidden />
              Thèmes de retraites suggérés
            </p>
            <ul className="space-y-2 text-sm leading-relaxed text-padma-night/80">
              {retreats.map((line) => (
