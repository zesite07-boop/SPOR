...
export function TreasureSimScenario({ hyperfocus }: { hyperfocus?: boolean }) {
  ...
  return (
    <div className={cn("space-y-4", hyperfocus && "space-y-3")}>
      ...
      <div>
        <label htmlFor="scenario-template" className="font-display text-[0.65rem] uppercase tracking-[0.18em] text-padma-pearl dark:text-padma-lavender/85">
          Modèle
        </label>
        <select
          ...
          className="mt-1.5 w-full rounded-xl border border-padma-champagne/25 bg-white/90 px-3 py-2.5 text-sm text-padma-night outline-none focus:ring-2 focus:ring-padma-lavender/35 dark:border-padma-lavender/25 dark:bg-padma-night/60 dark:text-padma-cream"
        >
      ...
      <div className="rounded-xl border border-padma-champagne/18 bg-gradient-to-r from-padma-champagne/12 to-padma-lavender/10 px-4 py-4 dark:border-padma-lavender/14 dark:from-padma-champagne/8 dark:to-padma-lavender/8">
        <p className="text-xs text-padma-night/70 dark:text-padma-cream/72">
          Projection ponctuelle sur ce cycle : {scenario.retreatDays} jour(s),{" "}
          <span className="font-medium text-padma-night dark:text-padma-cream">{scenario.projectedHeads}</span> participante(s), panier{" "}
          {pkg.priceEuro} €.
        </p>
        <p className="mt-2 font-cinzel text-2xl tracking-wide text-padma-night dark:text-padma-cream">
