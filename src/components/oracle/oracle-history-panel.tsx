...
export function OracleHistoryPanel({ refreshKey = 0 }: { refreshKey?: number }) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-padma-night/72">
        Historique complet des tirages du module Oracle — stocké uniquement dans ton navigateur (Dexie).
      </p>
      <ul className="space-y-3">
        {rows.length === 0 && (
          <li className="rounded-2xl border border-dashed border-padma-pearl/50 px-4 py-8 text-center text-sm text-padma-night/55">
        ...
        {rows.map((s) => (
          <motion.li
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="rounded-2xl border border-padma-champagne/30 bg-white/75 p-4"
          >
            <div className="flex flex-wrap items-start justify-between gap-2">
              <div>
                <p className="font-cinzel text-sm text-padma-night">{s.title}</p>
              ...
              <Button
                ...
                variant="ghost"
                size="icon"
                className="shrink-0 text-padma-night/45 hover:text-red-600"
            ...
            <p className="mt-2 line-clamp-4 text-sm leading-relaxed text-padma-night/78">{s.interpretation}</p>
          </motion.li>
        ))}
      </ul>
    </div>
