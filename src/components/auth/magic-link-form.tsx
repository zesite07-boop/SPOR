...
export function MagicLinkForm({ allowedEmails = [] }: { allowedEmails?: string[] }) {
  ...
  return (
    <form onSubmit={(e) => void onSubmit(e)} className="flex flex-col gap-6">
      {authErrorHint && (
        <p className="rounded-2xl border border-red-200/80 bg-red-50/90 px-4 py-3 text-center text-sm text-red-900/90">
          {authErrorHint}
        </p>
      )}

      <div className="space-y-2">
        <label htmlFor="padma-email" className="block text-xs font-medium tracking-wide text-padma-night/65">
          Ton e-mail
        </label>
        <div className="relative">
          <Mail
            className="pointer-events-none absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-padma-lavender"
            aria-hidden
          />
          <input
            id="padma-email"
            ...
            disabled={loading}
            className="w-full rounded-2xl border border-padma-champagne/45 bg-white/95 py-3.5 pl-11 pr-4 text-sm text-padma-night shadow-inner outline-none ring-0 transition-[border-color,box-shadow] placeholder:text-padma-night/35 focus:border-padma-lavender/80 focus:shadow-[0_0_0_3px_rgba(197,180,212,0.25)]"
          />
        </div>
      </div>
      ...
      {adminEmail && (
        <div className="space-y-2">
          <label htmlFor="padma-totp" className="block text-xs font-medium tracking-wide text-padma-night/65">
          ...
          <input
            ...
            placeholder="000000"
            className="w-full rounded-2xl border border-padma-champagne/45 bg-white/95 py-3.5 px-4 text-center text-xl tracking-[0.34em] text-padma-night outline-none focus:border-padma-lavender/80"
          />
          <p className="text-xs text-padma-night/55">
            Modules réservés — entrez votre code d&apos;authentification.
          </p>
          {twofaConfigured === false && (
            <div className="rounded-xl border border-padma-champagne/35 bg-padma-cream/45 p-3 text-xs text-padma-night/72">
          ...
          {showSetup && (
            <div className="space-y-2 rounded-xl border border-padma-lavender/35 bg-white/75 p-3">
              ...
              <p className="break-all rounded-lg bg-white/70 px-2 py-1 text-[10px] text-padma-night/70">
                Secret: {setupSecret}
              </p>
              <input
                inputMode="numeric"
      ...
      {!supabaseConfigured && (
        <p className="text-center text-xs leading-relaxed text-padma-night/55">
          Variables <code className="rounded bg-padma-pearl/25 px-1 py-0.5 text-[0.65rem]">NEXT_PUBLIC_SUPABASE_*</code> absentes
      ...
      <p className="text-center text-[11px] leading-relaxed text-padma-night/45">
