...
function ConfirmationInner() {
  ...
  if (!mounted) {
    return <p className="text-sm text-padma-night/65 dark:text-padma-cream/70">Chargement…</p>;
  }
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-lg text-center">
      ...
      <h1 className="font-cinzel text-2xl text-padma-night dark:text-padma-cream">Merci de ta confiance</h1>
      <p className="mt-4 text-sm leading-relaxed text-padma-night/78 dark:text-padma-cream/82">
      ...
      {sessionId && (
        <p className="mt-3 font-mono text-[0.65rem] text-padma-night/45 dark:text-padma-cream/45">Réf. session : {sessionId}</p>
      )}
      ...
      <p className="mt-10 flex items-center justify-center gap-2 text-xs text-padma-night/50 dark:text-padma-cream/55">
